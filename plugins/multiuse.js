...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'image') { await message.sendMessage(data, MessageType.image, { caption: "MADE BY HASHIR ☘️" }) }
    else if (type === 'video') { await message.sendMessage(data, MessageType.video, { caption: "MADE BY HASHIR ☘️" }) }
});
//const axios = require('axios')
